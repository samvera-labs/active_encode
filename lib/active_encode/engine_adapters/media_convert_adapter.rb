...
module ActiveEncode
  module EngineAdapters
    class MediaConvertAdapter
      ...
      attr_accessor :log_group, :queue, :role, :output_bucket
      ...
      def cancel(id)
        response = mediaconvert.cancel_job(id: id)
        find(id)
      end
      ...
      def get_encode_results(job)
        ...
        end_time = job.timing.finish_time || Time.now
      ...
      def event_rule_exists?(rule_name)
        rule = cloudwatch_events.list_rules(name_prefix: rule_name).rules.find do |rule|
          rule.name == rule_name
        end
        !rule.nil?
      end
